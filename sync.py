# sync.py - 从fix目录同步核心文件到根目录（仅文件大小不同时同步）
import os
import shutil
import printk
...
# 定义必须同步的核心文件
REQUIRED_FILES = [
    "kernel.py", "main.py", "fs.py", "printk.py", "sync.py", "btcfg.py"  # 新增btcfg.py同步
]
# 定义必须同步的目录（递归同步）
REQUIRED_DIRS = ["apps", "etc"]  # 新增etc目录（bootcfg存储目录）
